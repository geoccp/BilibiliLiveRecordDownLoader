...
namespace LiveRecordDownLoader
{
	public partial class App
	{
		...
		public App()
		{
			...
			{
#if DEBUG
				var identifier = $@"Global\{nameof(LiveRecordDownLoader)}_Debug";
#else
				var identifier = $@"Global\{nameof(LiveRecordDownLoader)}";
#endif
			...
			{
				MessageBox.Show($@"WTF??? {ex}", nameof(LiveRecordDownLoader), MessageBoxButton.OK, MessageBoxImage.Error);
				Environment.Exit(1);
			}
		}
		...
		private void UnhandledException(Exception ex)
		{
			...
			{
				Log.Fatal(ex, @"未捕获异常");
				MessageBox.Show($@"未捕获异常：{ex}", nameof(LiveRecordDownLoader), MessageBoxButton.OK, MessageBoxImage.Error);
			}
			finally
			{
				AppExit(1);
